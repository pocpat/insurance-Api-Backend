...
test("returns 'toyota' for model with string",() =>{
   ...
   test("returns false for empty input",() =>{
    expect(validateModel('')).toEqual({ valid: false, message: "Model must be a non-empty string containing only letters, numbers, and spaces" });
});
test("returns false for model with numbers only",() =>{
    expect(validateModel('8739')).toEqual({ valid: false, message: "Model must be a non-empty string containing only letters, numbers, and spaces" });
});
